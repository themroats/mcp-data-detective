...
def _generate_script(recipe: ChartRecipe) -> str:
    ...
    if recipe.color:
        lines += [
            ...
            f'    g = str(row["{recipe.color}"])',
            "    if g not in groups:",
            "        groups[g] = ([], [])",
            f'    groups[g][0].append(row["{recipe.x}"])',
            f'    groups[g][1].append(row["{recipe.y}"])',
            "for label, (gx, gy) in groups.items():",
            f'    ax.{recipe.chart_type}(gx, gy, label=label, alpha=0.8)',
            "ax.legend()",
        ]
    else:
        if recipe.chart_type == "bar":
            lines.append(f'ax.bar(df["{recipe.x}"], df["{recipe.y}"], alpha=0.8)')
        elif recipe.chart_type == "line":
            lines.append(f'ax.plot(df["{recipe.x}"], df["{recipe.y}"], marker="o", markersize=3)')
        elif recipe.chart_type == "scatter":
            lines.append(f'ax.scatter(df["{recipe.x}"], df["{recipe.y}"], alpha=0.6)')
        elif recipe.chart_type == "histogram":
            lines.append(f'ax.hist(df["{recipe.y}"], bins="auto", alpha=0.8, edgecolor="black")')

    slug = recipe.name.lower().replace(" ", "_")
    lines += [
        ...
        f'output = Path("charts") / "{slug}.png"',
        "output.parent.mkdir(parents=True, exist_ok=True)",
        'fig.savefig(str(output), dpi=150, bbox_inches="tight")',
        "plt.close(fig)",
        f'print(f"Chart saved to {{output}}")',
    ]
